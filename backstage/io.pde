    ...
    for(int n = 0; n < jsonNodes.size(); n++) {
      ...
      switch(node.getString("type")) {
        case "Link":
          ...
          node.getInt("index"), node.getInt("x"), node.getInt("y"), node.getJSONArray("next").getIntArray()));
          break;
        case "Rect":
          ...
          node.getFloat("nX"), node.getFloat("nY"), node.getFloat("nW"), node.getFloat("nH"),  node.getFloat("beginTransitionDuration"), node.getFloat("endTransitionDuration"),
...
void saveProject(File file) {
  if (file != null) {
    ...
    for(Node no: nodes) {
      ...
      jsonNode.setInt("index", no.index); jsonNode.setInt("x", no.x); jsonNode.setInt("y", no.y);
...
void insertMedia(int x, int y, File file) {
  ...
  boolean loop = false; boolean beginPaused = false; boolean endPaused = false; boolean beginTransition = false; boolean endTransition = false;
  float duration = defaultDuration; float beginTransitionDuration = 1; float endTransitionDuration = 1;
  int beginTransitionType = 0; int endTransitionType = 0; float nX = 0; float nY = 0; float nW = 1; float nH = 1;
  boolean aspectRatio = true; boolean centered = true;
  float volume = 1; float beginAt = 0; float endAt = 0;

  switch(type) {
    case "Image":
      nodes.add(new Image(label, notes, duration, beginPaused, endPaused, index, x, y, next,
      path,
      loop, beginTransition, endTransition, centered, aspectRatio,
      nX, nY, nW, nH, beginTransitionDuration, endTransitionDuration,
      beginTransitionType, endTransitionType));
      break;
    case "Audio": 
      nodes.add(new Audio(label, notes, duration, beginPaused, endPaused, index, x, y, next,
      path,
    ...
    case "Video": 
      ...
      path,
      loop, beginTransition, endTransition, centered, aspectRatio,
      nX, nY, nW, nH, beginTransitionDuration, endTransitionDuration, volume, beginAt, endAt,
      beginTransitionType, endTransitionType));
      break;
  }
}